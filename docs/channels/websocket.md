...
| `Ctrl+P` → `p` | Toggle minimal progress mode (on by default): the chat progress tail collapses to a single animated line (`⠹ 12.4s · bash` — spinner, loop elapsed, current tool), and the completed-turn thinking line is left out of the history |
