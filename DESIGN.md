...
17. **Inspect Server + Dashboard** — HTTP REST endpoints (`/state`, `/reload_config`, `/reset_session`, `/inject_message`, `/health`) plus WebSocket chat at `/ws[/<channel>]`, TUI dashboard for live monitoring
...
│                       │    axum HTTP REST             │   (ratatui TUI)      │
│  ┌─────────────────┐ │                                │                      │
│  │ InspectServer    │ │   GET  /state                 │  polls every 500ms   │
│  │ [inspect] config │ │  ◄──────────────────────────  │  reqwest HTTP client │
│  │                  │ │   InspectState JSON           │  auto-reconnect      │
│  │ queries:         │ │  ──────────────────────────►  │                      │
│  │  ThreadManagers  │ │                                │                      │
│  │  MetricsCollector│ │   POST /inject_message        │  WebSocket:          │
│  │  AuthMiddleware  │ │   ...                         │  GET /ws/<channel>   │
│  └─────────────────┘ │                                │                      │
└──────────────────────┘                                 └──────────────────────┘
```
...
The inspect server is a single axum HTTP router:
...
| `POST` | `/reset_session` | `ResetSessionRequest` | `ResetSessionResult` |
| `POST` | `/inject_message` | `InjectMessageRequest` | `InjectMessageResult` |
| `GET` | `/ws[/<channel>]` | — | WebSocket upgrade |

Error responses use proper HTTP status codes (400/404/401/500) with
`{"error":"…"}` JSON bodies.

### Authentication

Auth is **opt-in via file presence** at `<data_dir>/inspect-token` (e.g.
`~/.local/share/jyc/inspect-token` on Linux). Rules:
...
There is **no loopback bypass** — if you've enabled auth (by creating the
token file), you must authenticate even from `127.0.0.1`. The file is
read fresh on every connection so `jyc token rotate` takes effect
immediately for new connections. Constant-time compare via
`subtle::ConstantTimeEq`. The token is generated by `jyc token generate`
(format: `jyc_<64 hex chars>`, 256 bits of entropy, file mode `0600`).
...
The inspect server speaks HTTP via `axum`. Same TCP port as before
(`127.0.0.1:9876` default) — only the wire format changed.
...
Action endpoints use `POST` with a typed JSON body. Error responses are
proper HTTP status codes (400/404/401/500) with `{"error":"…"}` JSON
bodies. See the **Endpoints** table above for the full API surface.
...
│       │                                                          │
│       │  HTTP POST /inject_message (same reqwest client)         │
│       ▼                                                          │
│  InspectServer::handle_inject_message                             │
│       │                                                          │
│       │  body: { channel, thread, text }                         │
│       ▼                                                          │
...
| Inject via inspect HTTP `POST /inject_message`, not WebSocket | Non-WS channels have no WebSocket handler; the same HTTP client used for `/state` polling handles injection |
...
│   │   ├── server.rs                 # HTTP inspect server (axum router)
