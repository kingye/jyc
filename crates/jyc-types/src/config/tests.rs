...
"#;
    ...
    #[test]
    fn test_agent_overlay_applies_at_load() {
        let toml = r#"
[general]
...
"#;
        ...
        let config = load_config_from_str(toml).unwrap();
        let p = &config.channels["work"].patterns.as_ref().unwrap()[0];
        // Agent fields fill in where the pattern leaves them unset
        assert_eq!(p.template.as_deref(), Some("invoice"));
        assert_eq!(p.skills, Some(vec!["invoice-processing".to_string()]));
        // Pattern's own explicit value wins over the agent's
        assert_eq!(p.model.as_deref(), Some("pattern/model"));
    }
