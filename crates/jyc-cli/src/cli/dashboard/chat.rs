...
/// Width of the right-aligned agent-mode chip ("[ BUILD ]" / "[ PLAN ] ").
const MODE_CHIP_WIDTH: u16 = 9;
...
pub(super) fn render_chat_conversation(frame: &mut Frame, area: Rect, app: &mut App) {
    ...
    // Subtract the prompt gutter and mode chip from the wrap width.
    let input_line_count = count_wrapped_lines(
        &app.chat.text(),
        area.width
            .saturating_sub(PROMPT_GUTTER_WIDTH + MODE_CHIP_WIDTH),
    )
    .clamp(1, 10) as u16;
    let chunks = Layout::default()
        .direction(Direction::Vertical)
    ...
    // editor (it dims when the message area is focused); a filled
    // agent-mode chip ("[ BUILD ]" / "[ PLAN ]") is right-aligned on the
    // first editor row.
    ...
    let [prompt_area, editor_area, chip_area] = Layout::horizontal([
        Constraint::Length(PROMPT_GUTTER_WIDTH),
        Constraint::Min(0),
        Constraint::Length(MODE_CHIP_WIDTH),
    ])
    .areas(chunks[1]);
    ...
    // Always-visible agent-mode chip, right-aligned on the first editor
    // row (green bg = build, yellow bg = plan); defaults to build when
    // the thread's mode is unset. Sourced from the polled overview, same
    // as the Thread Info pane.
    let thread_mode = app
        .state
        .as_ref()
        .and_then(|s| {
    ...
    pub(super) fn page_size(&self) -> usize {
        ...
        match self.focus {
            ChatFocus::ChatPane | ChatFocus::MessageArea => {
                ...
                // Subtract the prompt gutter and mode chip from the width.
                let input_lines = count_wrapped_lines(
                    &self.text(),
                    term_width.saturating_sub(PROMPT_GUTTER_WIDTH + MODE_CHIP_WIDTH),
                )
                .clamp(1, 10);
