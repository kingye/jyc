...
3. Copy template files to the thread directory (if not already present):
   ...
   ```
4. Generate the invoice list Excel using invoice_export:
   ```
   invoice_export --month YYYY-MM --output ./
   ```
   This creates `invoice_list_<month>.xlsx` in the current directory.
5. For monthly summary (if needed):
   - **With closing:** `invoice_close --month YYYY-MM` (generates both Excel + ZIP archive)
   - **Without closing:** Use `invoice_export --month YYYY-MM --output ./` again with different format option
