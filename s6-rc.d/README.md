...
## Integration with System-wide s6 Installation

These config files are versioned in the project and copied to the system-wide s6 installation at runtime:

**External location**: `/etc/s6-rc/s6-rc.d/`

**Startup flow** (`start-jyc.sh`):
1. Copies `s6-rc.d/jyc/` → `/etc/s6-rc/s6-rc.d/jyc/`
2. Creates `/etc/s6-rc/s6-rc.d/user/contents.d/jyc`
3. Initializes s6 supervision: `s6-rc-init /run/service`
4. Brings up services: `s6-rc -u /run/service`
...
|--------|--------|-----------|
| Config location | `/etc/s6-overlay/s6-rc.d/` | `s6-rc.d/` (in project) → `/etc/s6-rc/s6-rc.d/` (runtime) |
| Config management | Installed at image build time | Copied from project at runtime |
| Runtime state | `/run/service/jyc/` | `/run/service/jyc/` |
| Binary location | `/usr/local/bin/jyc` | `/home/jiny/projects/jyc/jyc` |
| s6 installation | In container | System-wide (/) |
