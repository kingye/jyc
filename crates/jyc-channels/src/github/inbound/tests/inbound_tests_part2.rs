use super::inbound_tests::{make_message, make_message_with_rules};
use crate::github::inbound::*;
use jyc_types::{ChannelMatcher, ChannelPattern, LabelRule, PatternMatch, PatternRules};
...
fn test_ci_timed_out_triggers_failure() {
    ...
    let has_failure = check_runs.iter().any(|cr| {
        cr.conclusion.as_deref() == Some("failure") || cr.conclusion.as_deref() == Some("timed_out")
    });
