...
//! the URL-scoped topic name from `/ws/<channel>/<topic>` to the inner
//! handler.
//!
//! Used by the inspect server to expose a topic-scoped URL for
//! **websocket-type channels**. The inner handler can use the `scoped_topic`
//! parameter to populate per-connection state (e.g. `Message.topic` fallback)
//! without requiring the client to repeat the topic in the payload.
//!
//! For non-websocket channels, the inspect server uses `TopicProxyHandler`
//! instead — see `topic_proxy.rs`.
...
impl WebsocketHandler for ScopedWsHandler {
    async fn handle(
        ...
        tracing::debug!(
            addr = %addr,
            topic = scoped_topic.unwrap_or("?"),
            "ScopedWsHandler: delegating to inner handler with scoped_topic"
        );
        self.inner.handle(ws, addr, scoped_topic).await
    }
}
