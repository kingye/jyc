...
description: Build and deploy jyc on bare metal using scripts/deploy.sh with nohup. Use when instructed to build, deploy, or build-and-deploy jyc.
---
...
WARNING: Process will be killed and restarted automatically by scripts/deploy.sh.
...
Step 2: Execute the deploy command FIRST (it returns immediately, deployment runs in background):
   ```
   systemd-run --user --unit=jyc-deploy-$(date +%s) --working-directory=$(pwd)/jyc bash ./scripts/deploy.sh
   ```
   This runs scripts/deploy.sh as a separate systemd transient unit, outside the jyc service cgroup,
...
- Do NOT run scripts/deploy.sh directly with `bash ./scripts/deploy.sh` — it MUST use systemd-run to survive the service restart
- Do NOT use "systemctl --user stop jyc" or "systemctl --user start jyc" or "systemctl --user restart jyc" directly — always use scripts/deploy.sh
...
2. Run Deploy steps above (reply first, then scripts/deploy.sh, then STOP)
...
CRITICAL: These are observation-only commands. For deployment, ALWAYS use scripts/deploy.sh as specified above.
