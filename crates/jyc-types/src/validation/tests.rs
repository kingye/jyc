...
/// A pattern's pipe target must name a configured websocket hub channel.
#[test]
fn test_pipe_target_must_be_websocket_hub() {
    let base = r#"
[hub.local_dev]
...
"#;
    ...
    // Valid: target is a websocket hub.
    let config = load_config_from_str(&pipe("local_dev")).unwrap();
    let errors = validate_config(&config);
    assert!(
        !errors.iter().any(|e| e.path.contains("pipe")),
        "valid pipe target must pass, got: {errors:?}"
    );

    // Invalid: target not configured at all.
    let config = load_config_from_str(&pipe("nowhere")).unwrap();
    let errors = validate_config(&config);
    assert!(
        errors
            .iter()
            .any(|e| e.path.contains("pipe.hub") && e.message.contains("not configured")),
        "unknown pipe target must fail, got: {errors:?}"
    );

    // Invalid: target exists but is an adapter (not a hub).
    let toml = format!(
        "{base}\n[adapters.other]\ntype = \"feishu\"\n\n[adapters.other.feishu]\napp_id = \"a\"\napp_secret = \"b\"\n"
    );
    let toml = format!(
        "{toml}\n[[adapters.feishu_bot.patterns]]\nname = \"piped\"\npipe = {{ hub = \"other\" }}\n"
    );
    ...
    assert!(
        errors
            .iter()
            .any(|e| e.path.contains("pipe.hub") && e.message.contains("not a websocket hub")),
        "non-hub pipe target must fail, got: {errors:?}"
    );
}
