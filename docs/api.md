...
Fallback: a `?token=<token>` query parameter is accepted in place of the
header, because browsers cannot set headers on `WebSocket` upgrades. The
web UI (`GET /`, §2.3) uses this only for its `/ws/*` connections; REST
calls use the header.

Exceptions NOT gated by the bearer middleware:

- `GET /` — the web UI static shell (no data; every API call it makes is
  gated).
- `/exchange/*` — access control there is a per-thread `?token=` query
  parameter generated by the `jyc_publish_file` tool (see §2.4), so share
  links work for end users who have no dashboard token.

### 2.3 Route table

| Method | Path                                          | Purpose                                            |
|--------|-----------------------------------------------|----------------------------------------------------|
| GET    | `/`                                           | Web UI dashboard (static shell, no bearer auth).   |
