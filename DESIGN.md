...
Loopback (`127.0.0.0/8`, `::1`, `::ffff:127.0.0.1`) bypasses auth
entirely. Non-loopback requests (including WebSocket upgrades) require
`Authorization: Bearer <token>` matching the contents of
`<data_dir>/inspect-token`. The token file is read fresh on every
connection — no in-memory cache — so `jyc token rotate` takes effect
